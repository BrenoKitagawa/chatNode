...
const Registrar = () => {
  ...
  async function submit(data) {
    try {
      // Remover ReSenha antes de enviar para o backend
      const body = {
        nome: data.nome,
        email: data.email,
        senha: data.senha
      ...
      <div>
        <form onSubmit={handleSubmit(submit)} className='flex flex-col gap-4 min-w-70 md:w-100'>
          ...
          <div>
            ...
            <Input
              type="password"
              id="ReSenha" placeholder="Re-Senha"
              {...register("ReSenha")}
            />
            {errors.ReSenha && (
              <span className="text-red-500 text-sm">{errors.ReSenha.message}</span>
            )}
          </div>

          <Button type="submit" className="mt-5 w-full">Registrar</Button>

          <Link to="/">
            <Button className="mt-2 w-full">Voltar</Button>
          </Link>
        </form>
      </div>
    </div>
